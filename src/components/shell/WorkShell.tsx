...
import { LayoutDashboard, FolderOpen, ClipboardList, Wallet, Menu, X, LogOut, Search, FileText, Upload, CircleHelp, UserRound, type LucideIcon } from 'lucide-react';
...
const links: { to: string; label: TKey; icon: LucideIcon }[] = [
  { to: '/work', label: 'contractor.nav.dashboard', icon: LayoutDashboard },
  { to: '/work/projects', label: 'contractor.nav.projects', icon: FolderOpen },
  ...
  return (
    <div className="min-h-screen bg-brand-off-white text-brand-near-black dark:bg-[#141414] dark:text-white">
      ...
      <header className="sticky top-0 z-30 flex h-20 items-center border-b border-brand-border-grey bg-[#ffffff] dark:border-[#2c2c2c] dark:bg-[#1e1e1e]">
        ...
        <Form action="/work" method="get" className="relative mx-6 hidden max-w-lg flex-1 lg:block">
      ...
      <aside className={cn('fixed bottom-0 left-0 top-20 z-40 w-60 flex-col overflow-y-auto border-r border-brand-border-grey bg-[#ffffff] p-4 dark:border-[#2c2c2c] dark:bg-[#1e1e1e] md:flex', open ? 'flex' : 'hidden')}>
        ...
        <div className="mt-auto pt-12">
          <Link to="/help" onClick={() => setOpen(false)} className="flex items-center gap-3 border-t border-brand-border-grey pt-5 text-sm font-medium dark:border-[#2c2c2c]"><CircleHelp className="size-5" />{t('nav.help')}</Link>
          <p className="mt-3 text-xs leading-relaxed text-brand-mid-grey">{t('contractor.dashboard.subtitle')}</p>
        </div>
      </aside>
      <main id="work-content" className="min-w-0 px-4 py-6 sm:px-6 md:ml-60 lg:px-8"><div className="mx-auto max-w-[1500px]">{children}</div></main>
    </div>
  );
}
