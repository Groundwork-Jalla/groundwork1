...
export default function ProtectionSteps() {
  return (
    <section className="bg-brand-near-black py-28 px-7 overflow-hidden">
      <div className="max-w-[1100px] mx-auto">
        <Reveal className="text-center mb-20">
          <span className="text-xs font-semibold tracking-[0.12em] text-white/40">HOW JALLA PROTECTS YOU</span>
          <h2 className="font-['Playfair_Display'] text-5xl font-medium text-white mt-3">
          ...
          {steps.map((step, i) => {
            ...
            return (
              <Reveal key={step.title} direction={illustrationLeft ? "left" : "right"} delay={0.05}>
                <div className={`flex flex-col md:items-center gap-10 md:gap-16 ${illustrationLeft ? "md:flex-row" : "md:flex-row-reverse"}`}>
                  <div className="flex-1 min-h-[300px] flex items-center">
                    <step.Scene />
                  </div>
                  <div className="flex-1">
                    <div className="font-['Playfair_Display'] text-[80px] leading-none text-white/10 font-medium">
                      0{i + 1}
                    </div>
                    <h3 className="text-2xl font-bold text-white mt-2">{step.title}</h3>
                    <p className="text-sm text-white/50 mt-3 leading-relaxed max-w-[360px]">{step.desc}</p>
                  </div>
                </div>
              </Reveal>
            );
