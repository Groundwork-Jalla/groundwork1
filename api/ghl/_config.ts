...
export const GHL_KEYS = [
  ...
  'GHL_CLIENT_SECRET',
  /**
   * The public keys GoHighLevel signs Marketplace webhooks with (Phase 6.2 A.0.1, 06 §16.10).
   *
   * A Marketplace app's webhooks (`InboundMessage` among them) are sent by GHL's platform,
   * not by a workflow, so they cannot carry our `X-Groundwork-Secret` header. GHL signs
   * them instead, over the exact request bytes, with a private key only GHL holds:
   *
   *   X-GHL-Signature — Ed25519, the current standard → GHL_WEBHOOK_PUBLIC_KEY_ED25519
   *   X-WH-Signature  — RSA-SHA256, legacy (deprecated 1 Sep 2026, may still be sent
   *                     during the transition)          → GHL_WEBHOOK_PUBLIC_KEY_RSA
   *
   * Both are PEM blocks copied from GHL's developer documentation (Webhook Integration
   * Guide) by a person and pasted into app_config. They are not secrets (they are
   * published), but they are what a forged webhook would need to defeat, so they are
   * configuration, never literals in code.
   *
   * Either unset — that door is closed (401 for requests presenting only that header).
   * Both unset and no inbound secret — the endpoint is 503, not open.
   */
  'GHL_WEBHOOK_PUBLIC_KEY_ED25519',
