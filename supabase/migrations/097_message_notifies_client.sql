...
--
-- Notify the sender. 013's INSERTs already exclude `p_sender_id`, and this does the same
-- by construction: it notifies the conversation's PERSON, and staff are never that. An
-- internal note is never mentioned — it is staff-only by definition (091) and must not
-- leak its existence through a notification or an email subject line.
--
-- Email is best-effort and separate from the bell. A Resend outage must not roll back a
-- message that is already stored and already on the client's screen. Same ordering and
-- same reason as 074 and 076.
--
...
BEGIN
  ...
  IF NOT FOUND OR v_conv.person_id IS NULL THEN RETURN NEW; END IF;

  -- Never the author. A staff member who is also somebody's client still does not get
  -- told about their own message.
  IF v_conv.person_id = NEW.sender_id THEN RETURN NEW; END IF;

  -- A thread with no project is a person-level conversation (an inbound WhatsApp, say).
  -- It is still worth a bell, but the email below names a project, so it is skipped.
  ...
  SELECT COALESCE(NULLIF(full_name, ''), email), email
    INTO v_name, v_email
    FROM public.profiles WHERE id = v_conv.person_id;

  -- ── The bell ───────────────────────────────────────────
  -- First and in its own block: the in-app notification is the one that must not be lost
  -- to an email problem.
  BEGIN
    ...
    VALUES (
      v_conv.person_id,
      'message_received',
      CASE WHEN v_project.id IS NULL THEN 'New message from Groundwork'
           ELSE 'New message about ' || COALESCE(NULLIF(v_project.name, ''), 'your project') END,
  ...
  -- ── The email ──────────────────────────────────────────
  IF v_email IS NULL OR v_email = '' OR v_project.id IS NULL THEN RETURN NEW; END IF;
  ...
  -- The message itself is NOT put in the email. A project message can carry a decision, a
  -- cost or a photo request, and email is the one place Groundwork does not control who
  -- ends up reading it. The mail says a message is waiting and where to read it.
...
-- AFTER INSERT, so a failure here can never roll back the message. Runs after 091's
-- `messages_advance_conversation`, which is BEFORE/AFTER on the same table but concerns
-- the conversation's status rather than anybody's inbox.
...
COMMENT ON FUNCTION public.notify_client_of_message() IS
  'Tells the conversation''s person that staff wrote to them — bell always, email when the '
  'thread names a project. Outbound only: inbound is theirs and internal is staff-only. '
  'Never notifies the sender. Best-effort; a mail failure never fails the message. See 097.';
